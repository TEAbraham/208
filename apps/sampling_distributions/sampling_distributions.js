...
function centerOnDistribution(dist) {
  ...
  try {
    ...
    // You might want to force redraw to recompute y-scale based on new curve height
    draw_population(); // redrawing population curve will automatically update y_scale_clt

    draw_theoretical(); // redraw theoretical if needed

    // Redraw histogram and samples based on new x-scale
    draw_histogram();
  } catch (e) {
...
function clt() {
  ...
  function zoomed_clt(event) {
    ...
    draw_histogram();
    draw_theoretical();
    // xAxisPopulation.call(d3.axisBottom(x_scale_clt));
    // xAxisHistogram.call(d3.axisBottom(x_scale_clt));
  }
...
$(window).load(function () {
  $(document).ready(function() {
    ...
    $("#descriptionTable").css("display", "table");
    $("#resetDist").css("display", "inline-block");
    $('.distributions').val('bernoulli');
    centerOnDistribution('bernoulli');
    draw_population();
    draw_theoretical();
 });
