...
function discrete_continuous() {
  ...
  function centerOnDistribution(dist) {
    ...
    if (dist === "normal") {
      ...
      const std = +document.getElementById("normalStd").value;
      newDomain = [mean - 5 * std, mean + 5 * std];
  
    } else if (dist === "uniform") {
      const min = +document.getElementById("uniformMin").value;
      const max = +document.getElementById("uniformMax").value;
      newDomain = [min - 1, max + 1];
  
    } else if (dist === "beta") {
      newDomain = [-0.2, 1.2];
  
    } else if (dist === "gamma") {
      const shape = +document.getElementById("gammaShape").value;
      const scale = +document.getElementById("gammaScale").value;
      const maxX = shape * scale + 5 * Math.sqrt(shape) * scale;
      newDomain = [-1, maxX];
      ...
      const k = +document.getElementById("chisquareDof").value;
      newDomain = [-1, k * 3];
  
    } else if (["poisson", "binomialDiscrete", "negbin"].includes(dist)) {
      newDomain = [-20, 20];
    ...
    xScaleDist.domain(newDomain);
    xDist.call(xAxisDist.scale(xScaleDist));
    yDist.call(yAxisDist.scale(yScaleDist));
    redrawPath(dist);
  }
  

  $(".inputDist").on("input", function(e) {
    let id = this.parentNode.id.replace(/^parameters-/, '');  // <-- FIX: remove 'parameters-' prefix
    centerOnDistribution(id);
...
function clt() {
  ...
  var width = 1000;//parseInt(d3.select("#graph").style("width")) - margin.left - margin.right,
      height = 400;
  // create svg
  ...
  var y_scale_clt = d3.scaleLinear().domain([0, 3]).range([height - (2*y1), 0]);
  ...
  // create three bars
  svg_clt.call(draw_bar, y1, "population");
  svg_clt.call(draw_bar, y1+y2, "samples");
  svg_clt.call(draw_bar, 3*y1, "sampling distribution");
  ...
  function sample() {
    if (!sampling) {
      sampling = true;
      samplingInterval = setInterval(tick, 100);  // draws every 100ms
  ...
  $(document).on("input", ".inputDist", function() {
    ...
    // Redraw population if a parameter slider is moved
    draw_population();
  });
