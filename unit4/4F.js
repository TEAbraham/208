...
function distributions() {
  ...
  const bernoulliWidth = fullWidth * 0.8;
  const bernoulliHeight = 500;
  ...
  let bernoulliBars = bernoulliSvg.selectAll(".bar")
    ...
    .attr("fill", (_, i) => i === 1 ? "#00d0a1" : "#FF9B3C"); // B is green, A is orange
  ...
  const binomialWidth = fullWidth * 0.8;
  const binomialHeight = 500;
...
function showTooltip(event, d) {
  ...
  tooltip.html(`k = ${d.k}<br>Probability = ${(d.prob * 100).toFixed(2)}%`)
    .style("left", (event.pageX + 10) + "px")
    .style("top", (event.pageY - 25) + "px");
    
  d3.select(this).attr("fill", "#00d0a1");  // Highlight color
}

function hideTooltip(event, d) {
  tooltip.transition().duration(200).style("opacity", 0);
  d3.select(this).attr("fill", "#64bdff");  // Default bar color
}

  // Update bars with tooltip events
  bars.enter()
    .append("rect")
    .attr("class", "bar")
