...
var unitlist = new Array("bp", "cp", "pd", "fi", "bi", "ra");
var unit_list = new Array("basic-probability", "compound-probability", "probability-distributions", "frequentist-inference", "bayesian-inference", "regression-analysis");
...
function modalTitleOnLoad() {
    ...
    $('.modal-unit-titles li').on("dblclick", function() {
        var cur_id = $(this).attr("id").slice(0, 2);
        var a = unitlist.indexOf(cur_id);

        window.location.href = "../" + unit_list[a] + "/" + end_url;

    })

    $('.modal-unit-titles li').on("click", function() {
        $('.modal-unit-titles li').removeClass('unit-highlighted');
        $(this).addClass('unit-highlighted');
        var cur_id = $(this).attr("id").slice(0, 2);
...
function titleChangeToUnit() {

    if (unit_name == true) {
        $("#display-unit").css("display", "none");
        $("#seeing-theory").css("display", "block");
    } else {
        $("#seeing-theory").css("display", "none");
        $("#display-unit").css("display", "block");
    }
...
function scrollTo() {
    ...
    $(".nav-unit-wrapper-s").click(function() {
        ...
        if (current_page) {
            ...
            var a = unitlist.indexOf(parent_id);
            unit_list[a];

            toNewUnitUnit(this, "../" + unit_list[a] + "/" + end_url);
    ...
    function toNewUnitUnit(thisObj, unit) {
        var url = unit;
...
function unitBackgroundColorChange() {
...
function displayCurrentClass() {
    var cur_unit = getCurrentUnit();
    $('#' + cur_unit).css("display", "block");
...
function getCurrentUnit() {
    return $('meta[name=unit]').attr("content");
}
